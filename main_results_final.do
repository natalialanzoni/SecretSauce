...
* All paths below are relative to the repo folder that holds this do-file and
* final_for_analysis.dta. Set Stata's working directory to that folder before
* running, e.g.:
*     cd "/path/to/SecretSauce"
* (or replace c(pwd) below with that folder's absolute path).
global root "`c(pwd)'"

global datapath "$root"

capture confirm file "$datapath/final_for_analysis.dta"
if _rc {
	display as error "final_for_analysis.dta not found in: $datapath"
	display as error "cd to the folder containing this do-file, then re-run."
	exit 601
}
...
global resultspath "$root/output/$sample_def"
